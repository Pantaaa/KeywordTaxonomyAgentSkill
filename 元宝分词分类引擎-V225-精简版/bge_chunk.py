...
def main():
    ...
    # V225h: 自动探测模型路径（精简版 skill 不含模型，从外部引用）
    model_path = None
    for cand in [os.path.join(os.path.dirname(os.path.abspath(__file__)), 'bge_model'),
        ...
        if cand and os.path.exists(os.path.join(cand, 'model.safetensors')):
            ...
            break
    if not model_path:
        print('[part] ❌ 未找到 BGE 模型（model.safetensors），请设置 BGE_MODEL_DIR 或使用 D:\\元宝分词分类引擎-V225\\bge_model', file=sys.stderr)
        return
    print(f'[part{part}] 模型路径: {model_path}')
